...
STEP_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5.step"
CSV_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_validation.csv"
AUDIT_CSV_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_interference_audit.csv"
COLOR_MANIFEST_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_color_manifest.csv"
REPORT_OUT = REPORT_DIR / "cadquery_rough_assembly_v5_report.md"
AUDIT_REPORT_OUT = REPORT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_interference_report.md"
COLOR_REPORT_OUT = REPORT_DIR / "color_export_attempt_report_v5.md"
...
MAIN_COMPONENTS = [
    ...
    ComponentSpec(
        ...
        note="v5 scan station moved away from gantry center",
    ),
    ComponentSpec(
...
def build_tube_layout() -> list[tuple[str, TubeSpec, tuple[float, float, float], str]]:
    ...
    layout.append(
        (
            ...
            (-90.0, 150.0, 35.0),
            "v5 scan tube moved away from gantry center and scanner body",
        )
    )
...
def make_label_plate(label: str) -> list[tuple[str, cq.Shape, str]]:
    ...
    subparts = [("plate_base", base, "plate_dark")]
    try:
        text_shape = (
            cq.Workplane("XZ")
            .text(label, fontsize=8.0 if len(label) <= 2 else 7.0, distance=0.8, combine=False, halign="center", valign="center")
            .translate((0, -2.0, 0))
            .val()
        )
        subparts.append(("raised_text", text_shape, "plate_text"))
    except Exception:
        marker_width = min(width - 8.0, 42.0)
        marker = cq.Workplane("XY").box(marker_width, 0.8, 3.0).translate((0, -2.0, 0)).val()
        subparts.append(("light_marker_fallback", marker, "plate_text"))
    return subparts


def generated_layout() -> list[tuple[str, list[tuple[str, cq.Shape, str]], tuple[float, float, float], tuple[float, float, float], str]]:
    adapter = cq.Workplane("XY").box(95.0, 6.0, 145.0).val()
    layout = [
        (
            "xz_adapter_plate_simplified",
            [("adapter_plate", adapter, "adapter_gray")],
            (0.0, -60.0, 218.0),
            (0.0, 0.0, 0.0),
            "v5 resized/shifted simplified X-Z adapter plate",
        )
    ]
    label_specs = [
        ("label_plate_INPUT", "INPUT", (-160.0, 225.0, 15.0)),
    ...
    for item_a, item_b in combinations(instances, 2):
        ...
        if bbox_is_candidate:
            ...
            if overlap_volume is None:
                minimum_distance, note = exact_distance(item_a.world_shape, item_b.world_shape)
                if note:
                    notes.append(note)
            elif overlap_volume <= OVERLAP_VOLUME_THRESHOLD_MM3:
                minimum_distance, note = exact_distance(item_a.world_shape, item_b.world_shape)
                if note:
                    notes.append(note)
        elif bbox_gap < threshold:
            minimum_distance, note = exact_distance(item_a.world_shape, item_b.world_shape)
            if note:
                notes.append(note)

        if allowed and (bbox_is_candidate or (minimum_distance is not None and minimum_distance < threshold)):
            status = "allowed_mount_contact"
        elif overlap_volume is not None and overlap_volume > OVERLAP_VOLUME_THRESHOLD_MM3:
            status = "overlap"
        elif minimum_distance is not None and 0.0 < minimum_distance < threshold:
            status = "too_close"
        elif bbox_gap < threshold and minimum_distance is None and not bbox_is_candidate:
            status = "too_close"
            notes.append("exact distance unavailable; bbox clearance below threshold")
        else:
            status = "ok"
        ...
        rows.append(
            {
                ...
                "minimum_distance_mm": "" if minimum_distance is None else f"{minimum_distance:.6f}",
...
def write_reports(
    ...
    AUDIT_REPORT_OUT.write_text(
        "\n".join(
            [
                "# CadQuery Rough Assembly v5 Interference Report",
                "",
...
def main() -> int:
    ...
    assembly = cq.Assembly(name="blood_sorting_robot_cadquery_rough_layout_v5")
    manifest_rows: list[dict[str, object]] = []
