...
STEP_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_2.step"
CSV_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_2_validation.csv"
AUDIT_CSV_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_2_interference_audit.csv"
COLOR_MANIFEST_OUT = OUT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_2_color_manifest.csv"
REPORT_OUT = REPORT_DIR / "cadquery_rough_assembly_v5_2_report.md"
AUDIT_REPORT_OUT = REPORT_DIR / "blood_sorting_robot_cadquery_rough_layout_v5_2_interference_report.md"
COLOR_REPORT_OUT = REPORT_DIR / "color_export_attempt_report_v5_2.md"
...
MAIN_COMPONENTS = [
    ...
    ComponentSpec(
        ...
        note="v5.2 grouped with scanner bracket and scan tube",
    ),
    ComponentSpec(
        ...
        note="v5.2 grouped with sensor bracket and scan tube",
    ),
]
...
def write_reports(
    ...
    REPORT_OUT.write_text(
        "\n".join(
            [
                "# CadQuery Rough Assembly v5.2 Report",
                "",
                "- v5.2 is an alignment correction based on v5.1, not a structural redesign.",
                "- Fixed Z-axis tool-chain alignment: X axis center, X-Z adapter plate, vertical Z axis, and SMC gripper are now on a clearer installation chain.",
                "- Regrouped scan station: barcode scanner, scanner bracket, photoelectric sensor, sensor bracket, and scan demo tube are now arranged as one station near the input rack.",
                "- Bottom plate, dual Y axes, X axis span, racks, bins, tube family, and label plate scheme remain based on v5.1.",
                ...
                "- STEP color export: attempted through CadQuery/OCP XCAF/STEPCAF; color manifest remains available.",
                ...
                "- Next step: open the v5.2 STEP in SolidWorks 2026 for manual inspection.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    AUDIT_REPORT_OUT.write_text(
        "\n".join(
            [
                "# CadQuery Rough Assembly v5.2 Interference Report",
                "",
    ...
    COLOR_REPORT_OUT.write_text(
        "\n".join(
            [
                "# Color Export Attempt Report v5.2",
                "",
                ...
                *[f"- {note}" for note in color_notes],
                "",
                "Conclusion: STEP color export was attempted again for v5.2. The color manifest remains the fallback if SolidWorks color handling changes.",
                "",
            ]
        ),
        encoding="utf-8",
...
def main() -> int:
    ...
    assembly = cq.Assembly(name="blood_sorting_robot_cadquery_rough_layout_v5_2")
    manifest_rows: list[dict[str, object]] = []
